/*
 * @(#)BasicTreeTableHeaderUI.java	1.53 00/02/02
 *
...
/**
 * BasicTreeTableHeaderUI implementation
 *
 * @version 1.53 02/02/00
 * @author Alan Chung
 * @author Philip Milne
 */
public class BasicTreeTableHeaderUI extends TreeTableHeaderUI {
...
// Instance Variables
//
    /** The TreeTableHeader that is delegating the painting to this UI. */
    protected TreeTableHeader header;

    protected CellRendererPane rendererPane;

    // Listeners that are attached to the TreeTable
...
//            if (source instanceof TreeTableHeader) {
//                TreeTableHeader th = (TreeTableHeader) source;
//                BasicTreeTableHeaderUI ui =
//                        (BasicTreeTableHeaderUI) BasicLookAndFeel.getUIOfType(th.getUI(),
//                        BasicTreeTableHeaderUI.class);
//                if (ui == null) {
//                    return;
//                }
//
...
//    };
    ...
    public class MouseInputHandler implements MouseInputListener {
        ...
        public void mouseClicked(MouseEvent e) {
            TreeTableColumnModel model = header.getColumnModel();
            if ( model.isSortable() && header.getDraggedColumn() == null && header.getResizingColumn() == null ) {
                int columnIndex = model.getColumnIndexAtX(e.getPoint().x);
                if ( columnIndex != -1 ) {
                    TreeTableColumn column = model.getColumn(columnIndex);
                    if ( column.isSortable() ) {
                        TreeTableColumn currentSortColumn = model.getSortColumn();
                        if ( currentSortColumn == column ) {
                            model.setSort( column, ! model.isSortAscending());
                        }
                        else {
        ...
        private TreeTableColumn getResizingColumn(Point p) {
            return getResizingColumn(p, header.columnAtPoint(p));
        }

        private TreeTableColumn getResizingColumn(Point p, int column) {
            if (column == -1) {
                return null;
            }
        ...
        public void mousePressed(MouseEvent e) {
            ...
            TreeTableColumnModel columnModel = header.getColumnModel();
            int index = header.columnAtPoint(p);

            if (index != -1) {
                // The last 3 pixels + 3 pixels of next column are for resizing
                TreeTableColumn resizingColumn = getResizingColumn(p, index);
                if (canResize(resizingColumn, header)) {
                    ...
                    TreeTableColumn hitColumn = columnModel.getColumn(index);
        ...
        public void mouseDragged(MouseEvent e) {
            ...
            TreeTableColumn resizingColumn = header.getResizingColumn();
            TreeTableColumn draggedColumn = header.getDraggedColumn();
            ...
            if (resizingColumn != null) {
                ...
                TreeTableColumnModel cm = header.getColumnModel();
...
//
    /**
     * Creates the mouse listener for the TreeTableHeader.
     */
...
//
    public static ComponentUI createUI(JComponent h) {
        return new BasicTreeTableHeaderUI();
    }

//  Installation
    public void installUI(JComponent c) {
        header = (TreeTableHeader) c;
    ...
    /**
     * Initialize TreeTableHeader properties, e.g. font, foreground, and background.
    ...
    protected void installDefaults() {
        ...
        UIManager.put( "Tree.forwardSortIcon", new ImageIcon( getClass().getResource("/treeTable/sortUpIcon.png") ));
        UIManager.put( "Tree.backwardSortIcon", new ImageIcon( getClass().getResource("/treeTable/sortDownIcon.png") ));
    }

    /**
     * Attaches listeners to the TreeTableHeader.
     */
    ...
    /**
     * Register all keyboard actions on the TreeTableHeader.
     */
...
//        LazyActionMap.installLazyActionMap(header, BasicTreeTableHeaderUI.class,
...
//
     ...
     * @see TreeTableHeader#getHeaderRect(int)
     * @since 1.6
     */
...
//
    ...
    private void scrollToColumn(int col) {
        ...
        if ((header.getParent() == null) ||
                ...
                ((table = header.getTreeTable()) == null)) {
            return;
        }

        //Now scroll, if necessary.
        Rectangle vis = table.getVisibleRect();
        Rectangle cellBounds = table.getColumnBounds(0, col);
        vis.x = cellBounds.x;
    ...
    private static boolean canResize(TreeTableColumn column,
            TreeTableHeader header) {
        return (column != null) && header.getResizingAllowed() && column.getResizable();
    }

    private int changeColumnWidth(TreeTableColumn resizingColumn,
            TreeTableHeader th,
        ...
        if (!container.getComponentOrientation().isLeftToRight() &&
            ...
            if ((newHeaderWidth >= viewportWidth) &&
                    (table.getAutoResizeMode() == TreeTable.AUTO_RESIZE_OFF)) {
...
//
    ...
    public int getBaseline(JComponent c, int width, int height) {
        ...
        for (int column = 0; column < columnModel.getColumnCount();
                column++) {
            TreeTableColumn aColumn = columnModel.getColumn(column);
...
//
    ...
    private Component getHeaderRenderer(int columnIndex) {
        TreeTableColumn aColumn = header.getColumnModel().getColumn(columnIndex);
        TreeTableCellRenderer renderer = aColumn.getHeaderRenderer();
        if (renderer == null) {
            renderer = header.getDefaultRenderer();
        }

      //  boolean hasFocus = !header.isPaintingForPrint() && (columnIndex == getSelectedColumnIndex()) && header.hasFocus();
        return renderer.getTreeTableCellRendererComponent(header.getTreeTable(),
                aColumn,
                false, false, false,
                -1, columnIndex, false);
    }
...
//
    ...
    public Dimension getMinimumSize(JComponent c) {
        ...
        while (enumeration.hasMoreElements()) {
            TreeTableColumn aColumn = (TreeTableColumn) enumeration.nextElement();
...
//            if (sender instanceof TreeTableHeader) {
//                TreeTableHeader th = (TreeTableHeader) sender;
//                TreeTableColumnModel cm = th.getColumnModel();
...
//            TreeTableHeader th = (TreeTableHeader) e.getSource();
//            BasicTreeTableHeaderUI ui =
//                    (BasicTreeTableHeaderUI) BasicLookAndFeel.getUIOfType(th.getUI(),
//                    BasicTreeTableHeaderUI.class);
//            if (ui == null) {
...
//        }
//
//        private void moveColumn(boolean leftArrow, TreeTableHeader th,
//                BasicTreeTableHeaderUI ui) {
//            maybeMoveColumn(leftArrow, th, ui, true);
//        }
//
//        private boolean maybeMoveColumn(boolean leftArrow, TreeTableHeader th,
//                BasicTreeTableHeaderUI ui, boolean doIt) {
...
//        }
//
//        private void resize(boolean leftArrow, TreeTableHeader th,
//                BasicTreeTableHeaderUI ui) {
//            int columnIndex = ui.getSelectedColumnIndex();
//            TreeTableColumn resizingColumn =
...
//        }
//    }
}  // End of Class BasicTreeTableHeaderUI
