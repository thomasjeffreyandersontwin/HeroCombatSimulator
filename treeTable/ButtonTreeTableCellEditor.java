 ...
 * @author  1425
 */
public class ButtonTreeTableCellEditor extends JButton implements TreeTableCellEditor, TreeTableCellRenderer {
    
    protected TreeTableButtonNode editorNode;
    protected TreeTableButtonNode rendererNode;
    ...
    public ButtonTreeTableCellEditor() {
      ...
      /*  Font font = UIManager.getFont("AbilityTree.defaultFont");
        ...
        this.addActionListener( new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                TreeTableButtonNode oldNode = editorNode;
                int oldColumn = editorColumn;
                if ( editorTree != null ) {
                    if ( ! editorTree.stopEditing() ) {
                        editorTree.cancelEditing();
                    }
                }
                if ( oldNode != null ) oldNode.actionPerformed(oldColumn, e);
            }
        });
    ...
    public Component getTreeTableCellEditorComponent(TreeTable treeTable, Object node, boolean isSelected, boolean expanded, boolean leaf, int row, int column){ 
        ...
        value = treeTable.getProxyTreeTableModel().getValueAt(node, column);
        ...
        if(node instanceof TreeTableButtonNode) {
            editorNode = (TreeTableButtonNode)node;
        }
        
        if( editorNode != null ) {
            setEnabled( editorNode.isColumnEnabled(column));
            
            font = editorNode.getColumnFont(column);
            color = editorNode.getColumnColor(column);
        }
    ...
    public Component getTreeTableCellRendererComponent(TreeTable treeTable, Object node, boolean isSelected, boolean expanded, boolean leaf, int row, int column, boolean hasFocus) {
        ...
        if(node instanceof TreeTableButtonNode) {
            rendererNode = (TreeTableButtonNode)node;
        }
        
        if( rendererNode != null ) {
            setEnabled( rendererNode.isColumnEnabled(column));
            
            font = rendererNode.getColumnFont(column);
            color = rendererNode.getColumnColor(column);
        }
