...
implements Serializable, MouseListener {
    ...
    protected boolean drawHorizonalLine = false;
    ...
    public TabControl() {
        ...
        outline = UIManager.getColor("TabbedPane.darkShadow");
        disabled = UIManager.getColor("TabbedPane.shadow");
        highlight = UIManager.getColor("TabbedPane.lightHighlight");
    ...
    public void paintComponent(Graphics g) {
        ...
        int top = insets.top;
        int right = s.width - insets.right - 1;
        int bottom = s.height - insets.bottom - 1;
        ...
        if ( height < f.getHeight() ) {
            fontYPos = bottom;
        }
        else {
            fontYPos = ( height + f.getAscent()) / 2 - 1 ;
        }
        ...
        for (int t = firstVisible; t <= lastVisible; t++) {
            int tabWidth = ((Integer) sizes.elementAt(t)).intValue();
            
            if ( t == selectedIndex ) {
                g.setColor(getBackground());
                g.fillRect(xPosition,top,tabWidth,height);
            }
            else {
                g.setColor(disabled);
                g.fillRect(xPosition,top,tabWidth,height);
                ...
                if ( direction == DIR_DOWN ) {  
                    g.fillRect(xPosition,bottom-4,6,5);
                }
                else {
                    g.fillRect(xPosition,top,6,5);
                }
                
                g.setColor(disabled);
                
                if ( direction == DIR_DOWN ) {
                    g.drawLine(xPosition+4,bottom-4,xPosition+5,bottom-4);
                    g.drawLine(xPosition+5,bottom-3,xPosition+5,bottom-3);
                }
                else {
                    g.drawLine(xPosition+4,top+4,xPosition+5,top+4);
                    g.drawLine(xPosition+5,top+3,xPosition+5,top+3);
                }
            ...
            if ( t == firstVisible ) {
                if ( direction == DIR_DOWN ) {
                    g.drawLine( xPosition, top, xPosition, bottom - 6);
                }
                else {
                    g.drawLine( xPosition, top+6, xPosition, bottom);
                }
            }
            else {
                if ( direction == DIR_DOWN ) {
                    g.drawLine( xPosition, top, xPosition, bottom);
                }
                else {
                    g.drawLine( xPosition, top, xPosition, bottom);
                }
            ...
            if ( direction == DIR_DOWN ) {
                g.drawLine( xPosition, bottom - 6, xPosition + 6, bottom);
                g.drawLine( xPosition + 6, bottom, xPosition + tabWidth - 1, bottom);
            }
            else {
                g.drawLine( xPosition, top+6, xPosition + 6, top);
                g.drawLine( xPosition + 6, top, xPosition + tabWidth - 1, top);
            }
            
            if ( t == lastVisible ) {
                if ( direction == DIR_DOWN ) {
                    g.drawLine( xPosition + tabWidth - 1, bottom, xPosition + tabWidth - 1, top);
                }
                else {
                    g.drawLine( xPosition + tabWidth - 1, top, xPosition + tabWidth - 1, bottom);
                }
            }
            ...
            if ( direction == DIR_DOWN ) {
                g.drawLine( xPosition+1, top, xPosition+1, bottom - 6);
                g.drawLine( xPosition+1, bottom - 6, xPosition + 6, bottom - 1);
            }
            else {
                g.drawLine( xPosition+1, top+6, xPosition+1, bottom);
                g.drawLine( xPosition+1, top+6, xPosition + 6, top+1);
            }
            
            
            if ( t != selectedIndex && drawHorizonalLine ) {
                g.setColor( outline );
                if ( direction == DIR_DOWN  ) { 
                    g.drawLine(xPosition, top, xPosition + tabWidth, top);
                }
                else {
                    g.drawLine(xPosition, bottom, xPosition + tabWidth, bottom);
                }
            }
            ...
            String str = (String) tabs.elementAt(t);
            int delta = f.stringWidth(str);
            delta = ((tabWidth - delta - 2* (margin + indent)) / 2);
            g.drawString(str, xPosition + margin + indent + delta, fontYPos);
        ...
        if ( drawHorizonalLine && xPosition < width) {
            g.setColor( outline );
            if ( direction == DIR_DOWN ) { 
                g.drawLine(xPosition, top, right, top);
            }
            else { 
                g.drawLine(xPosition, bottom, right, bottom);
            }
        }
    }
    ...
    /** Getter for property drawHorizonalLine.
     * @return Value of property drawHorizonalLine.
     *
     */
    public boolean isDrawHorizonalLine() {
        return drawHorizonalLine;
    }
    
    /** Setter for property drawHorizonalLine.
     * @param drawHorizonalLine New value of property drawHorizonalLine.
     *
     */
    public void setDrawHorizonalLine(boolean drawHorizonalLine) {
        this.drawHorizonalLine = drawHorizonalLine;
    }
