 ...
 * @author  1425
 */
public class RosterNameTargetFilter implements Filter<Target> {
    
    /** substring which much match the ability name. */
    private String filterString = null;
    
    /** Creates a new instance of RangedAbilityFilter */
    public RosterNameTargetFilter() {
    }
    
    public RosterNameTargetFilter(String includeText) {
    ...
    public boolean includeElement(Target target) {
        return target != null && 
                (filterString == null || 
                 "".equals(filterString) ||
                 (target.getRoster() != null && target.getRoster().getName().toLowerCase().indexOf(filterString.toLowerCase()) != -1));
    }    
