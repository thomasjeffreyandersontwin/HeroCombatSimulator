 ...
 * @version
 */
public class ColumnLayout2 extends Object implements LayoutManager, Columned, PropertyChangeListener {
    ...
    public ColumnLayout2() {
        listenerList = new EventListenerList();
    }


    public ColumnLayout2(int columns,int width,int hgap,int vgap) {
        if ( columns == -1 ) {
            fixedColumns = false;
        }
        else {
    ...
    public Dimension preferredLayoutSize(Container parent) {
        ...
        Dimension a = parent.getSize();
        Dimension b;
        
        //Dimension b = parent.getPreferredSize();
       ...
       // System.out.println( "PrfInsets: " + insets.toString());
        ...
        if ( parent.getParent() instanceof JViewport ) {
          ...
          //  JScrollPane sp = (JScrollPane)parent.getParent().getParent();
            
            int vert = 0, hori = 0;
            vp.getInsets(insets);
            vert += insets.top + insets.bottom;
            hori += insets.left + insets.right;
            
          //  if ( sp != null ) {
                Dimension d = vp.getSize();
                if ( preferredSize.width < d.width - vert) preferredSize.width = d.width - vert;
                if ( preferredSize.height < d.height - hori) preferredSize.height = d.height - hori;
          //  }
        }
    ...
    public void layoutContainer(Container parent) {
        ...
        synchronized (parent.getTreeLock()) {
            ...
            if ( parent.getParent() instanceof JViewport ) {
                ...
                if ( currentSize.height < vpSize.height - vert ) {
                    currentSize.height = vpSize.height - vert;
                    changeSize = true;
                }
                if ( currentSize.width < vpSize.width - hori) {
                    currentSize.width = vpSize.width - hori;
                    changeSize = true;
                }
                if ( changeSize ) {
