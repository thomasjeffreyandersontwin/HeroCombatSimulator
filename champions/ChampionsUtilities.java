...
implements ChampionsConstants {
    ...
    static public void calculateDCs(BattleEvent be, String targetGroup) {
        ...
        baseAbilityAdvCost = ability.getAdvDCCost();
...
//        if ( isHeroic ) {
//            rstr = ustr = (minimumStr.intValue() >= normalStr.intValue()) ? 0 : normalStr.intValue() - minimumStr.intValue();
//            rpushed = pushedStrUsed = ( rstr > 0 ) ? pushedStr.intValue() : normalStr.intValue() + pushedStr.intValue() - minimumStr.intValue();
//        }
//        else {
//            rstr = ustr = normalStr.intValue();
//            rpushed = pushedStrUsed = pushedStr.intValue();
//        }
        
        
//        if ( be.isKillingAttack() ) {
//            if ( baseAbilityDC == null ) {
//                // This is an unarmed killing attack
//                realdc = Math.round( ( maneuverDC.doubleValue() + martialDC.doubleValue() / 2 + combatDC.doubleValue() ) * 5 / baseAbilityPointsPerDCWithAdv );
//                totalDC = realdc;
//                
//                // Adjust it according to otherDCModifiers
//                totalDC += otherDCModifiers;
//                
//                if ( be.strengthAddsToDC() ) {
//                    // Add the strength in.
//                    if ( Math.round( totalDC +  rstr * realStrMultiplier / baseAbilityPointsPerDCWithAdv ) >= realdc * 2 ) {
//                        ustr = (int)Math.round( ( realdc * 2 - totalDC ) / realStrMultiplier * baseAbilityPointsPerDCWithAdv );
//                    }
//                    else {
//                        ustr = rstr;
//                    }
//                    totalDC += ustr * realStrMultiplier / baseAbilityPointsPerDCWithAdv;
//                    
//                    // Add the pushedStr strength in
//                    if ( Math.round( totalDC +  rpushed * realStrMultiplier / baseAbilityPointsPerDCWithAdv ) >= realdc * 2 ) {
//                        pushedStrUsed = (int)Math.round( ( realdc * 2 - totalDC ) / realStrMultiplier * baseAbilityPointsPerDCWithAdv );
//                    }
//                    else {
//                        pushedStrUsed = rpushed ;
//                    }
//                    totalDC += pushedStrUsed * realStrMultiplier / baseAbilityPointsPerDCWithAdv;
//                }
//                else {
//                    ustr = 0;
//                    pushedStrUsed = 0;
//                }
//                
//                totalDC = Math.round( totalDC );
//            }
//            else {
//                // Killing with weapon...
//                realdc = baseAbilityDC.doubleValue();
//                totalDC = realdc;
//                
//                // Adjust it according to otherDCModifiers
//                totalDC += otherDCModifiers;
//                
//                // Add the Maneuver DC
//                //the '/2' was not here before...added by Pete due to rules on page 271 (top c2)
//                totalDC += maneuverDC.doubleValue() /2 * 5 / baseAbilityPointsPerDCWithAdv;
//                // Add the Martial levels in.
//                totalDC += martialDC.doubleValue() / 2 * 5 / baseAbilityPointsPerDCWithAdv;
//                // Add in Combat Levels
//                totalDC += combatDC.doubleValue() * 5 / baseAbilityPointsPerDCWithAdv;
//                if ( totalDC > realdc * 2 ) totalDC = realdc * 2;
//                
//                if ( be.strengthAddsToDC() ) {
//                    // Add the strength in.
//                    if ( Math.round( totalDC +  rstr * realStrMultiplier / baseAbilityPointsPerDCWithAdv ) >= realdc * 2 ) {
...
//                    }
//                    totalDC += ustr * realStrMultiplier / baseAbilityPointsPerDCWithAdv;
//                    if ( totalDC > realdc * 2 ) totalDC = realdc;
//                    
//                    // Add the pushedStr strength in
//                    if ( Math.round( totalDC +  rpushed * realStrMultiplier / baseAbilityPointsPerDCWithAdv ) >= realdc * 2 ) {
//                        pushedStrUsed= (int)Math.round( ( realdc * 2 - totalDC ) / realStrMultiplier * baseAbilityPointsPerDCWithAdv );
...
//                totalDC = Math.round( totalDC );
//            }
//            if ( dindex != -1 ) be.addIndexed(dindex, "Die", "SIZE" ,  DCToKillingString((int)totalDC), true );
//        }
//        else if ( isHeroic ) {
//            // Process a isHeroic, normal attack
//            if ( baseAbilityDC == null ) {
//                // This is an unarmed isHeroic normal attack
//                realdc = Math.round( ( maneuverDC.doubleValue() + martialDC.doubleValue() + combatDC.doubleValue() ) * 5 / baseAbilityPointsPerDCWithAdv );
//                totalDC = realdc;
//                
//                // Adjust it according to otherDCModifiers
//                totalDC += otherDCModifiers;
...
//                
//                // Add the Maneuver DC
//                totalDC += maneuverDC.doubleValue() * 5 / baseAbilityPointsPerDCWithAdv;
//                
//                // Add in Combat Levels
//                totalDC += combatDC.doubleValue() * 5 / baseAbilityPointsPerDCWithAdv;
//                
...
//                if ( totalDC > realdc * 2 ) totalDC = realdc * 2;
//                
//                // Add the Martial levels in (Can exceed 2x weapon damage).
//                totalDC += martialDC.doubleValue() * 5 / baseAbilityPointsPerDCWithAdv;
//                
//                totalDC = roundHalf( totalDC );
//            }
//            if ( dindex != -1 ) be.addIndexed(dindex, "Die","SIZE",  DCToNormalString((int)totalDC), true );
//        }
//        else {
//            // Normal Superheroic (armed or unarmed)
//            if ( baseAbilityDC == null ) {
//                realdc = 0;
//            }
//            else {
...
//            if ( be.strengthAddsToDC() ) {
//                // Add in Normal Str
//                totalDC += rstr * realStrMultiplier / baseAbilityPointsPerDCWithAdv;
//                
//                // Add in Pushed Str
//                totalDC += rpushed * realStrMultiplier / baseAbilityPointsPerDCWithAdv;
//            }
//            else {
//                ustr = 0;
//                pushedStrUsed = 0;
...
//        }
//        
//        // Adjust Used Str to reflect normalStr min in isHeroic
//        if ( isHeroic ) {
//            ustr += minimumStr.intValue();
//            if ( ustr > normalStr.intValue() ) {
//                pushedStrUsed += ustr - normalStr.intValue();
//                ustr = normalStr.intValue();
//            }
//        }
