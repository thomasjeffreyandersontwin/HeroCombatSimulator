 ...
 * @version
 */
public class VariationListNode extends AbilityTreeNode
        implements ChangeListener, AbilityInstanceGroupListener {
    
    /** Holds value of property detailList. */
    private AbilityInstanceGroup abilityInstanceGroup;
    
    private static JMenuItem removeMenuItem, addMenuItem;
    ...
    public VariationListNode(AbilityInstanceGroup abilityInstanceGroup, MutableTreeNode parent) {
        setParent(parent);
        setAbilityInstanceGroup(abilityInstanceGroup);
    ...
    public VariationListNode(AbilityTreeTableModel model, AbilityInstanceGroup abilityInstanceGroup, MutableTreeNode parent) {
        setModel(model);
        setParent(parent);
        setAbilityInstanceGroup(abilityInstanceGroup);
        setAllowsChildren(true);
...
//        }
   ...
   /* protected void buildNode() {
        ...
        if ( abilityInstanceGroup != null ) {
            ...
            count = abilityInstanceGroup.getVariationCount();
            for(index = 0; index < count; index++) {
                ...
                ability = abilityInstanceGroup.getVariation(index);
                if ( ability instanceof CombinedAbility ) {
                    mtn = new CombinedAbilityNode(model, null, this, ability);
                }
                else {
                    mtn = new AbilityNode(model, null, this, ability);
                }
                children.add(mtn);
                //}
            }
 ...
 /*   public SublistNode findSublistNode(TreeNode node, String newParentName) {
    ...
    public void updateChildren() {
        ...
        if ( abilityInstanceGroup != null ) {
            ...
            acount = abilityInstanceGroup.getVariationCount();
            for( aindex = 0; aindex < acount; aindex++) {
                ability = abilityInstanceGroup.getVariation(aindex);
                ...
                if ( found == false ) {
                    if ( ability instanceof CombinedAbility ) {
                        an = new CombinedAbilityNode(model, null, this, ability);
                        
                    } else {
                        an = new AbilityNode(model, null, this, ability);
                    }
                    an.setTree(tree);
                    newChildren.add(an);
                    fireChange = true;
            ...
            if ( found == false ) {
                CreateVariableInstanceNode cvin = new CreateVariableInstanceNode(model, this, abilityInstanceGroup.getCurrentInstance());
                cvin.setTree(tree);
                newChildren.add(cvin);
                fireChange = true;
            }
    ...
    public boolean canPasteData(Transferable t) {
        return false; //return t != null && t.isDataFlavorSupported(abilityFlavor);
    }
     ...
     /*   if ( abilityList != null ) {
            switch (columnIndex) {
                case AbilityTreeTableModel.ABILITY_TREE_PTSCOLUMN:
                    v =  Integer.toString( abilityList.getCPCost() );
                    break;
            }
        } */
    ...
    public TreeTableCellRenderer getTreeTableCellRenderer(int columnIndex) {
       /* if ( columnIndex == AbilityTreeTableModel.ABILITY_TREE_PTSCOLUMN ) {
            return renderer;
        }
        else {
            return null;
    ...
    public void abilityAdded(AbilityAddedEvent evt) {
        if ( evt.getAbility().isVariationInstance() ) {
            updateChildren();
        }
    }
