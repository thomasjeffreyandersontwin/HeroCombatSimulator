 ...
 * @version
 */
public class CombinedExecuteNode extends DefaultAttackTreeNode {
    
    /** Creates new AutofireAttackNode */
    public CombinedExecuteNode(String name) {
        this.name = name;
        setVisible(false);
    }
        ...
        if ( activeChild != null && activeChild.getName() == null ) {
            ...
            if ( nextNodeName != null ) {
                ...
                if ( index != -1 ) {
                    ...
                    CombinedActivateAbilityNode node = new CombinedActivateAbilityNode(nextNodeName);
                    node.setCombinedAbilityIndex(index);
                    nextNode = node;
                }
            }
        }
    ...
    public String nextNodeName(String previousNodeName) {
        ...
        CombinedAbilityBattleEvent lbe = (CombinedAbilityBattleEvent)battleEvent;
        int count = lbe.getCombinedAbilityCount();
        ...
        for(int i = previousIndex+1; i < count; i++) {
            if ( lbe.isCombinedAbilityEnabled(i) ) {
    ...
    public void adjustTarget(BattleEvent be, Target source, int referenceNumber, String targetGroup, int primaryTargetNumber) {
        ...
        int children = lbe.getCombinedAbilityCount();
        for(int index=1; index<children; index++) {
            BattleEvent aBE = lbe.getSubBattleEvent(index);
            if ( aBE == be ) {
                // This was the node.  Find the other battleEvent
                BattleEvent previousBE = lbe.getSubBattleEvent(index-1);
                ...
                if ( previousTargetGroup != null ) {
                    ...
                    if ( previousTargetIndex != -1 ) {
                        ...
                        String reason = "This target is a primary target from an earlier combined attack.\n\n" +
                                        "Primary targets must be the same for consecutive combined attacks.";
