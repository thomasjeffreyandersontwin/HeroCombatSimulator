 ...
 * @version 
 */
public class AttackParametersNode extends DefaultAttackTreeNode
implements ChangeListener {
    ...
    public static AttackParametersNode Node;
    ...
    public AttackParametersNode(String name) {
        this.name = name;
    ...
    public boolean activateNode(boolean manualOverride) {
        ...
        if ( nodeRequiresInput() || manualOverride == true ) {
            AttackParametersPanel app = AttackParametersPanel.getAttackParametersPanel( getBattleEvent() );
            ...
            attackTreePanel.setInstructions("Configure the options for the ability...");
    ...
    public boolean processAdvance() {
        ...
        if ( battleEvent.getBooleanValue("BattleEvent.SHOULDSHUTDOWN") == true ) {
            ActivationInfo ai = battleEvent.getActivationInfo();
            battleEvent.addAIStateEvent(ai, ai.getState(), AI_STATE_DEACTIVATING );
            ai.setState( AI_STATE_DEACTIVATING );
        }
    ...
    public String getAutoBypassOption() {
        return "SHOW_ATTACK_PARAMETERS_PANEL";
    }
    ...
    public void buildChildren() {
        // Build the subnode of the AttackParametersNode
        Ability ability = battleEvent.getAbility();
        if ( ability != null ) {
            AttackTreeNode node = ability.getPower().getSetupPowerNode(battleEvent, battleEvent.getSource());
            if ( node != null ) {
                this.addChild(node);
            }
        }
        ...
        if ( maneuver != null ) {
            AttackTreeNode node = maneuver.getPower().getSetupPowerNode(battleEvent, battleEvent.getSource());
            if ( node != null ) {
                this.addChild(node);
            }
        }
