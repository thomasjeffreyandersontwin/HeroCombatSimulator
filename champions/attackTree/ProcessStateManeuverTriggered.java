 ...
 * @version
 */
public class ProcessStateManeuverTriggered extends DefaultAttackTreeNode {
    
    private boolean childrenBuilt = false;
    
    /** Creates new ProcessActivateAbilityNode */
    public ProcessStateManeuverTriggered(String name) {
        this.name = name;
    ...
    public boolean processAdvance() throws BattleEventException {
        ...
        BattleEngine.processStateManeuverTriggered(battleEvent);
        
        if ( childrenBuilt == false ) {
            addChild( BattleEngine.buildPreactivationNodes(battleEvent, battleEvent.getManeuver() ) );
            childrenBuilt = true;
        }
