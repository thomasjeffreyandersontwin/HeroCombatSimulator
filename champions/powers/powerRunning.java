 ...
 */
public class powerRunning extends Power 
implements ChampionsConstants {
    ...
    private static String powerName = "Running"; // The Name of the Power
  ...
  */
    public boolean configurePAD(Ability ability, ParameterList parameterList) {
        ...
        ability.add("Ability.MOVETYPE", "Running", true);
     ...
     public int calculateCPCost(Ability ability) {
        ...
        if ( base ) {
            if ( distance.intValue() > 6 ) cost += (distance.intValue() - 6) * 2;
        }
        else {
            cost += (distance.intValue()) * 2;
        }
    ...
    public String getConfigSummary(Ability ability, int not_used) {
        ...
        if ( base ) {
            return "Running (+" + Integer.toString( distance.intValue() - 6 ) + "\", " + distance.toString() + "\", NC: " + Integer.toString( distance.intValue() * noncombatX.intValue() ) + "\")";
        }
        else {
            return "Running (" + Integer.toString( distance.intValue()  ) + "\"" + ( addsToBase ? " + base" : "") + ", NC: " + Integer.toString( distance.intValue() * noncombatX.intValue() ) + "\")";            
        }
    }
    ...
    public int identifyPower(Ability template, AbilityImport ai) {
        ...
        if ( power != null && ( power.equals( "RUNNING" ) || power.equals( "Running" ) )){
            return 10;
        }
        return 0;
    } 
    ...
    public int getMovementDistance(Ability ability) {
       ...
       // Integer noncombatX = (Integer)parameterList.getParameterValue("NoncombatX");
        ...
        if ( addsToBase ) {
            ...
            if ( source != null ) {
                Ability baseRunning = source.getAbilityList().getAbility("Running", true);
                if ( baseRunning.getPower() != this ) {
