 ...
 * @version 
 */
public class effectDrain extends Effect implements ChampionsConstants {  
    /** Creates new effectAdjustment */
    public effectDrain(Object adjustable, Target source, Ability sourceAbility, double adjustmentAmount, long decayInterval, int decayRate) {
        super( "DrainEffect", "INSTANT", false);
        setSource(source);
        ...
        if ( adjustable instanceof Ability ) {
            sindex = addDrainSubeffect("Drain", "ABILITY", adjustable, adjustmentAmount);
        }
        else {
            sindex = addDrainSubeffect("Drain", "STATCP", ((Characteristic)adjustable).getName(), adjustmentAmount);
        }
    ...
    public void setDrainLimit(double limit) {
        setLimit(0, limit);
    }
