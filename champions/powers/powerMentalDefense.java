 ...
 */
public class powerMentalDefense extends Power implements ChampionsConstants {
    ...
    static private Object[][] parameterArray = {
        ...
        {"AddToTotal","Power.ADDTOTOTAL", String.class, "FALSE", "Add To Total", STRING_PARAMETER, HIDDEN, ENABLED, NOTREQUIRED},
        
    };
    
    // Power Definition Variables
    private static String powerName = "Mental Defense"; // The Name of the Power
    ...
    private static Object[][] patterns = {
        ...
        { "(Add to Total)", new Object[] { "AddToTotal", Boolean.class}},
        
    };
    
    
    /** Creates new powerHandToHandAttack */
    public powerMentalDefense()  {
    }
  ...
  */
    ...
    public String getConfigSummary(Ability ability, int not_used) {
        ...
        return  "Mental Defense (+" +level.toString() + " MD)";
    }
    ...
    public int identifyPower(Ability template, AbilityImport ai) {
        ...
        if ( power != null && ( power.equals( "MENTALDEFENSE" ) || power.equals( "Mental Defense" ) )){
            return 10;
        }
        return 0;
    }
    ...
    public void triggerPower(BattleEvent be, Ability ability,DetailList effectList, Target target, int refNumber, String targetGroup) throws BattleEventException {
        ParameterList parameterList = getParameterList(ability);
        Integer level = (Integer)parameterList.getParameterValue("Level");
        //Integer md = (Integer)parameterList.getParameterValue("Md");
        ...
        int sourceego = 0;
        if (target != null) {
            sourceego = target.getCurrentStat( "EGO" );
        }
        int md =  (sourceego/5) + level.intValue();
        // Add Effects
       // int i = effect.addSubeffectInfo( "MD", "AID", "NONE", "NORMAL", "STAT", "MD", md );
        effect.addIncreaseSubeffect("MD", "MD", md, ADJUSTED_STAT);
    ...
    /** Imports informatino from the AbilityImport into a new Ability.
     *
     * Mental Defense requires a custom importPower routine so the EGO
     * can be factored out of the level provided by Creation Workshop.
     *
     */
    ...
    public void importPower(Ability ability, AbilityImport ai) {
        ...
        //grab the import lines again to get EGO stat value
        CharacterImport ci = (CharacterImport)ai.getValue("AbilityImport.CHARACTERIMPORT");
        
        // Grab the parameterList that was configured...
        ParameterList parameterList = getParameterList(ability);
        
        // Check to see if the Add to Value was marked.
        String addtototal = (String)parameterList.getParameterValue("AddToTotal");
        if ( addtototal.equals("TRUE") ) {
            
            // Find the location of the EGO information in the import
            int index = ci.findIndexed("Stat","NAME","EGO");
            
            if ( index != -1 ) {
                // Grab the EGO value and calculate the base MDF
                Integer ego = ci.getIndexedIntegerValue(index,"Stat","VALUE");
                int baseMD = ChampionsUtilities.roundValue( ego.intValue() / 5, true );
                
                // Grab the imported level and adjust via the calculated  base
                Integer importedMD = (Integer)parameterList.getParameterValue("Level");
                Integer adjustedMD = new Integer ( importedMD.intValue() - baseMD );
                // Save the adjusted value back out
                parameterList.setParameterValue("Level", adjustedMD);
            }
        }
    }
