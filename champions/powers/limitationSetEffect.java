 ...
 */
public class limitationSetEffect extends LimitationAdapter implements ChampionsConstants {
    ...
    static private Object[][] parameterArray = {
        {"Multiplier","Limitation#.MULTIPLIER", String.class, "-\u00BC", "Multiplier", COMBO_PARAMETER, VISIBLE, ENABLED, NOTREQUIRED, "OPTIONS", typeOptions}
    };
    
    // Advantage Definition Variables
    public static String limitationName = "Set Effect"; // The Name of the Advantage
    private static boolean unique = false; // Indicates whether multiple copies can be added to ability

    public limitationSetEffect() {
    }
    ...
    public String getConfigSummary() {
        ...
        String s = "set effect (" + multiplier + ")";
        return (s);
    ...
    public void importLimitation(Ability ability, int padIndex, AbilityImport ai, int lineIndex) {
        ...
        for(index=lineIndex;index<count;index++) {
            ...
            if ( ChampionsMatcher.matches( "(.*) (-.*)", line ) || ChampionsMatcher.matches( "(.*), (-.*)", line ) || ChampionsMatcher.matches( "(.*): (-.*)", line ) || ChampionsMatcher.matches( "(.*) \\((-.*)\\).*", line ) ) {
                ...
                int position = (int) Math.round(value / 0.25 * -1 - 1);
