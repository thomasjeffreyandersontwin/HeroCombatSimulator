 ...
 */
public class advantageAutofire extends AdvantageAdapter
implements ChampionsConstants {
    ...
    public static String advantageName = "Autofire"; // The Name of the Advantage
   ...
   // Import Patterns Definitions
    private static Object[][] patterns = {
        { "Autofire: ([0-9]*) shots,.*", new Object[] {"MaxShots", Integer.class}},
        { "Autofire \\(([0-9]*).*", new Object[] {"MaxShots", Integer.class}}

    };
    
    /** Creates new advCombatModifier */
    public advantageAutofire() {
    }
    ...
    public boolean configurePAD(Ability ability, ParameterList parameterList) {
        ...
        Integer maxshots = (Integer)parameterList.getParameterValue("MaxShots");
        ...
        ability.add("Ability.MAXSHOTS",  maxshots , true);
    ...
    public double calculateMultiplier() {
        ...
        max = 5;
        i = 0;
        while ( maxshots .intValue() > max ) {
            max *= 2;
            i ++;
        }
        
        double cost = 0.5 + (i * 0.25);
        
        Power p = ability.getPower();
        
        if ( p instanceof powerEgoAttack 
        || p instanceof powerDrain 
      //  || p instanceof powerTransfer
      //  || p instanceof powerTransform
        ) {
            cost *= 2;
        }
        else if ( 
            ability.hasAdvantage( PADRoster.getSharedAdvantageInstance( "Area Effect" ) )
        || ability.hasAdvantage( PADRoster.getSharedAdvantageInstance( "No Normal Defense" ) )
        || ability.hasAdvantage( PADRoster.getSharedAdvantageInstance( "Attack Vs. Limited Defense" ) )
        ) {
            cost *= 2;
        }
    ...
    public String getConfigSummary() {
        ...
        return "AF (" + maxshots.toString() + " shots)";
