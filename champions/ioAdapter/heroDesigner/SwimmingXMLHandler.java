...
import champions.ioAdapter.heroDesigner.adapters.powerSwimmingAdapter;
 ...
 * @author  1425
 */
public class SwimmingXMLHandler extends AbilityXMLHandler implements XMLHandler {
    
    /** Creates a new instance of AbilityXMLHandler */
    public SwimmingXMLHandler() {
    }
    
    public SwimmingXMLHandler(XMLParser parser) {
        super(parser);
    }
    ...
    public Object parseNode(Document doc, Node node, Object userData, XMLParseErrorList xmlParseErrorList) throws XMLParserException {
        ...
        if ( powerID == null || ! powerID.equals("SWIMMING")) {
            throw new XMLParserException("Attempt to import XMLID != SWIMMING by SwimmingXMLHandler");
        }
        
        PowerXMLAdapter pxa;
        if ( "CHARACTERISTICS".equals(node.getParentNode().getNodeName() )){
            pxa = new powerSwimmingAdapter(true);
        }
        else {
            pxa = new powerSwimmingAdapter(false);
        }
        Ability ability;
        ...
        if ( ability == null ) {
            ability = PADRoster.getNewAbilityInstance("Swimming");
        }
