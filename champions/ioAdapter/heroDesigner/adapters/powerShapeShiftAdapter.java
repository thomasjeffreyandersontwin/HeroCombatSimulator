 ...
 * @author  1425
 */
public class powerShapeShiftAdapter extends AbstractPowerXMLAdapter implements PowerXMLAdapter {
    
    private static String XMLID = "SHAPESHIFT";
    private static String[][] translationArray = {
        { "ADDER(XMLID=SHAPES).OPTION", "Shapes", "shapesSpecial"},
        ...
        { "ADDER(XMLID=CELLULAR)", "Cellular", "trueSpecial" },
        { "ADDER(XMLID=IMITATION)", "Imitation", "trueSpecial" },
        { "ADDER(XMLID=INSTANTCHANGE)", "InstantChange", "trueSpecial" },
        { "ADDER(XMLID=MAKEOVER)", "Makeover", "trueSpecial" },
//
...
//    }
    ...
    public void shapesSpecial(Ability ability, Node node, String attrValue, ParameterList pl, String parameterName, String specialData) {
        if ( "4".equals(attrValue) ) {
            pl.setParameterValue("Shapes", "Up to Four Shapes");
        } else if ( "LIMITED".equals(attrValue) ) {
            pl.setParameterValue("Shapes", "Limited Group of Shapes");
        } else if ( "ANY".equals(attrValue) ) {
            pl.setParameterValue("Shapes", "Any Shape");
        }
    }
