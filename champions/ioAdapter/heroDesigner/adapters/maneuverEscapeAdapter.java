 ...
 * @author  1425
 */
public class maneuverEscapeAdapter extends AbstractManeuverXMLAdapter implements ManeuverXMLAdapter {
    
    private static String[] displayArray = { "Martial Escape" };
    ...
    public boolean identifyXML(String XMLID, Node node) {
        if ( XMLID.equals("MANEUVER") ) {
            ...
            if ( e.equals("[STRDC] vs. Grabs") ) {
                return true;
            }
        }
        
        return super.identifyXML(XMLID, node);
    }
    ...
    public void dcSpecial(Ability ability, Node node, String attrValue, ParameterList pl, String parameterName, String specialData) {
        if (!attrValue.equals("0")) {
            try {
                int dc = Integer.parseInt(attrValue);
                pl.setParameterValue("Strength", dc * 5);
    ...
    public XMLParseError importXML(Ability ability, Node node) {
        ...
        if ( (result == null || result.getErrorSeverity() < HDImportError.IMPORT_FAILURE) && !ability.hasSpecialParameter(new SpecialParameterIsMartialManeuver())
        ) {
