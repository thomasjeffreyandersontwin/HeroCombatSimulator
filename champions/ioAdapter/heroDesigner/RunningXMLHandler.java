...
import champions.ioAdapter.heroDesigner.adapters.powerRunningAdapter;
 ...
 * @author  1425
 */
public class RunningXMLHandler extends AbilityXMLHandler implements XMLHandler {
    
    /** Creates a new instance of AbilityXMLHandler */
    public RunningXMLHandler() {
    }
    
    public RunningXMLHandler(XMLParser parser) {
        super(parser);
    }
    ...
    public Object parseNode(Document doc, Node node, Object userData, XMLParseErrorList xmlParseErrorList) throws XMLParserException {
        ...
        if ( powerID == null || ! powerID.equals("RUNNING")) {
            throw new XMLParserException("Attempt to import XMLID != RUNNING by BaseRunningXMLHandler");
        }
        
        PowerXMLAdapter pxa;
        if ( "CHARACTERISTICS".equals(node.getParentNode().getNodeName() )){
            pxa = new powerRunningAdapter(true);
        }
        else {
            pxa = new powerRunningAdapter(false);
        }
        Ability ability;
        ...
        if ( ability == null ) {
            ability = PADRoster.getNewAbilityInstance("Running");
        }
