 ...
 * @version
 */
public class AbsorbToAttackTreeNode extends DefaultAttackTreeNode {
    ...
    public AbsorbToAttackTreeNode(String name) {
        this.name = name;
    ...
    public boolean activateNode(boolean manualOverride) {
        ...
        AdjustmentList targetAL = (AdjustmentList)ai.getIndexedValue(tindex, "Target", "ABSORBTOLIST");
        
        if ( targetAL == null ) {
            setAdjustmentList(new AdjustmentList(target));
        } else {
            setAdjustmentList( (AdjustmentList)targetAL.clone());
        }
           ...
           for (; eindex >= 0; eindex--) {
               ...
               if ( effect instanceof effectAbsorption ) {
                   ...
                   if ( ea.getAbility().equals( battleEvent.getAbility() ) ) {
                        // Grab the targetAL from that
                   }
               }
           }*/
           /*effectAbsorbTracker eat = powerAbsorb.findAbsorbTracker(ai.getSource(), ai.getAbility());
            if ( eat == null ) {
                setAdjustmentList( new AdjustmentList(target) );
            }
            else {
        ...
        if ( nodeRequiresInput() || manualOverride == true ) {
            AdjustmentAttackTreePanel app = AdjustmentAttackTreePanel.getDefaultPanel(battleEvent, ADJ_CONFIG_ABSORB_TO, battleEvent.getAbility(), target, adjustables, adjustmentList);
            
            attackTreePanel.showInputPanel(this,app);
            attackTreePanel.setInstructions("Configure the Abilities/Stats of " + target.getName() + " to Absorb to...");
    ...
    public boolean processAdvance() {
        ...
        ai.addIndexed(tindex, "Target", "ABSORBTOLIST", adjustmentList, true);
