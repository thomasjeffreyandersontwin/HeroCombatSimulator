...
public class SimulatorSingleAttack extends AbilityWrapper {
	...
	public SimulatorSingleAttack(String name, CharacterAdaptor character) {
	...
	public void Export() {
		...
		ExportSingleAttackResults(attack, battleEvent,0);
		ExportSingleKnockback( attack, battleEvent,0);
		WriteJSON(attack);
...
protected  JSONObject ExportSingleAttackResults(JSONObject attackJSON, BattleEvent event, int index) {
	...
	ObstructionList ol = battleEvent.getActivationInfo().getObstructionList(index);
...
protected void ExportSingleKnockback(JSONObject attackJSON,BattleEvent event, int index) {
	...
	int  tindex=index;
	if(event.getActivationInfo().getTarget(0).getName().equals("Hex"))
	{
		tindex--;
	}
	int distance = event.getKnockbackAmount(tindex);
	knockback.put("Distance", distance);
	
	if(event.getKnockbackEffect(tindex)== KnockbackEffect.POSSIBLECOLLISION) {
